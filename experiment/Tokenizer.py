...
class PatternTokenizer(BaseTokenizer):
    ...
    def process_ds(self, ds):
        ...
        if self.remove_repetitions:
            ...
            ds = ds.str.replace(pattern, r"\1",regex=True)

        for target, patterns in self.patterns.items():
            for pat in patterns:
                prev_ds=ds.str
                ds = ds.str.replace(pat, target,regex=True)
                if prev_ds!=ds.str:
                    print(prev_ds,ds)
...
def main():
    ...
    test = pd.read_csv("../kaggle_data/test_x.csv")
    
    tokenizer = PatternTokenizer()
    
    train["string"] = tokenizer.process_ds(train["string"][:100]).str.join(sep=" ")
    val["string"] = tokenizer.process_ds(val["string"]).str.join(sep=" ")
    test["string"] = tokenizer.process_ds(test["string"]).str.join(sep=" ")
